...
int main(int argc, char *argv[]){
    ...
    cilk_scope{
        ...
        { if( (NCILK-1) >= 0 ) { cilk_spawn spawn_function(); }
            else{ clock_gettime(CLOCK_MONOTONIC, &t_end); cilk_sync; end_program(&t_start, &t_end, NCILK); return 0; } }
		...
		{ if( (NCILK-271) >= 0 ) { cilk_spawn spawn_function(); }
		 ...
		 { if( (NCILK-301) >= 0 ) { cilk_spawn spawn_function(); }
            else{ clock_gettime(CLOCK_MONOTONIC, &t_end); cilk_sync; end_program(&t_start, &t_end, NCILK); return 0; } } // 301
