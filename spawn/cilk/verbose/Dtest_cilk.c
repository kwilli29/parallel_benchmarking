...
int main(int argc, char *argv[]){
	...
	// cilk_scope {} ? 
	  ...
	  for(int j=1; j <= innerreps; j++){
		...
		// #pragma omp parallel for schedule (static, 1) // schedule -- static, dynamic, guided, grainsize
		#pragma cilk grainsize 1 // schedule?
		cilk_for(int i=1; i <= itersperthr*NCILK; i++){
			delay(delaylength);
		}
