...
make cilkfor-count_cilk
make for-count_cilk

EXECS=$(ls -p | grep -v / | grep -E '.*count_cilk$')
...
for var in $EXECS; do
	for num in "${THREADS[@]}"; do
		touch "output/run2/$(basename "$var")/$(basename "$var")_$num.txt"
		for (( i=0; i<$RUNS; i++ )); do
			# echo $(basename "$var")
			...
			echo "$num threads -- >= $res core" >> output/run2/$(basename "$var")/$(basename "$var")_$num.txt
			./$var $num >> output/run2/$(basename "$var")/$(basename "$var")_$num.txt
		done
